...
// MTConnectAssets is The root node for MTConnect
...
// MTConnectAssetsType is CuttingTool Information Model
...
// AssetsType is CuttingTool Information Model
type AssetsType struct {
	Asset []*AssetType `xml:"Asset"`
}

// AssetType is Item, thing or entity that has potential or actual value to an
...
// Asset is Item, thing or entity that has potential or actual value to an
//         organization. {{cite(ISO 55000:2014)}} * Note 1 to entry: Value can be
//         tangible or intangible, financial or non-financial, and includes
//         consideration of risks and liabilities. It can be positive or negative
//         at different stages of the asset life. * Note 2 to entry: Physical
...
//                   cycle data will not remain static, but will change 395
...
// CuttingTool is {{block(CuttingTool)}} {{block(Asset)}} type.
...
// CuttingToolArchetype is The {{block(CuttingToolArchetype)}} represents the static cutting tool
//         geometries and nominal values as one would expect from a tool catalog.
type CuttingToolArchetype *CuttingToolArchetypeType

// CuttingToolLifeCycleArchetypeType is {{block(CuttingItems)}} {{termplural(organize)}}
...
// CuttingToolLifeCycleType is {{block(CuttingItems)}} {{termplural(organize)}}
//             {{block(CuttingItem)}} elements.
...
// CuttingToolArchetypeReferenceType is {{block(CuttingToolArchetypeReference)}} has reference information about
...
// CutterStatusType is The status of the cutting tool.
type CutterStatusType struct {
	Status []string `xml:"Status"`
}

// LocationType is The Pot or Spindle the cutting tool currently resides in.
...
// ReconditionCountType is The number of times this cutter has been reconditioned.
type ReconditionCountType struct {
	MaximumCountAttr int `xml:"maximumCount,attr,omitempty"`
	Value            int `xml:",chardata"`
}

// ProcessSpindleSpeedType is The constrained process spindle speed for this tool.
...
// ProcessFeedRateType is The constrained process feed rate for this tool in mm/s.
...
// MeasurementType is A constrained scalar value associated with a cutting tool.
...
// Measurement is A constrained scalar value associated with a cutting tool.
...
// ProtrudingLengthType is The dimension from the yz-plane to the furthest point of the Tool Item
//         or Adaptive Item measured in the -X direction.
type ProtrudingLengthType struct {
	*CommonMeasurementType
}

// ProtrudingLength is The dimension from the yz-plane to the furthest point of the Tool Item
//         or Adaptive Item measured in the -X direction.
type ProtrudingLength *ProtrudingLengthType

// WeightType is The total weight of the Cutting Tool in grams. The force exerted by the
//         mass of the Cutting Tool.
type WeightType struct {
	*CommonMeasurementType
}

// Weight is The total weight of the Cutting Tool in grams. The force exerted by the
//         mass of the Cutting Tool.
type Weight *WeightType

// FunctionalLengthType is The distance from the gauge plane or from the end of the shank to the
...
// FunctionalLength is The distance from the gauge plane or from the end of the shank to the
//         furthest point on the tool, if a gauge plane does not exist, to the
//         cutting reference point determined by the main function of the tool. The
//         {{block(CuttingTool)}} functional length will be the length of the
//         entire tool, not a single Cutting Item. Each {{block(CuttingItem)}} can
...
// BodyDiameterMaxType is The largest diameter of the body of a Tool Item.
type BodyDiameterMaxType struct {
	*AssemblyMeasurementType
}

// BodyDiameterMax is The largest diameter of the body of a Tool Item.
type BodyDiameterMax *BodyDiameterMaxType

// BodyLengthMaxType is The distance measured along the X axis from that point of the item
...
// BodyLengthMax is The distance measured along the X axis from that point of the item
//         closest to the workpiece, including the Cutting Item for a Tool Item but
//         excluding a protruding locking mechanism for an Adaptive Item, to either
//         the front of the flange on a flanged body or the beginning of the
//         connection interface feature on the machine side for cylindrical or
...
// CuttingDiameterMaxType is The maximum diameter of a circle on which the defined point Pk of each
...
// CuttingDiameterMax is The maximum diameter of a circle on which the defined point Pk of each
//         of the master inserts is located on a Tool Item. The normal of the
//         machined peripheral surface points towards the axis of the Cutting Tool.
type CuttingDiameterMax *CuttingDiameterMaxType

// FlangeDiameterMaxType is The dimension between two parallel tangents on the outside edge of a
//         flange.
type FlangeDiameterMaxType struct {
	*AssemblyMeasurementType
}

// FlangeDiameterMax is The dimension between two parallel tangents on the outside edge of a
//         flange.
type FlangeDiameterMax *FlangeDiameterMaxType

// DepthOfCutMaxType is The maximum engagement of the cutting edge or edges with the workpiece
//         measured perpendicular to the feed motion.
type DepthOfCutMaxType struct {
	*AssemblyMeasurementType
}

// DepthOfCutMax is The maximum engagement of the cutting edge or edges with the workpiece
//         measured perpendicular to the feed motion.
type DepthOfCutMax *DepthOfCutMaxType

// OverallToolLengthType is The largest length dimension of the Cutting Tool including the master
//         insert where applicable.
type OverallToolLengthType struct {
	*AssemblyMeasurementType
}

// OverallToolLength is The largest length dimension of the Cutting Tool including the master
//         insert where applicable.
type OverallToolLength *OverallToolLengthType

// ShankDiameterType is The dimension of the diameter of a cylindrical portion of a Tool Item or
//         an Adaptive Item that can participate in a connection.
type ShankDiameterType struct {
	*AssemblyMeasurementType
}

// ShankDiameter is The dimension of the diameter of a cylindrical portion of a Tool Item or
//         an Adaptive Item that can participate in a connection.
type ShankDiameter *ShankDiameterType

// ShankHeightType is The dimension of the height of the shank.
type ShankHeightType struct {
	*AssemblyMeasurementType
}

// ShankHeight is The dimension of the height of the shank.
type ShankHeight *ShankHeightType

// ShankLengthType is The dimension of the length of the shank.
type ShankLengthType struct {
	*AssemblyMeasurementType
}

// ShankLength is The dimension of the length of the shank.
...
// CuttingDiameterType is The diameter of a circle on which the defined point Pk located on this
...
// CuttingDiameter is The diameter of a circle on which the defined point Pk located on this
//         Cutting Tool. The normal of the machined peripheral surface points
//         towards the axis of the Cutting Tool.
type CuttingDiameter *CuttingDiameterType

// CornerRadiusType is The nominal radius of a rounded corner measured in the X Y-plane.
type CornerRadiusType struct {
	*CuttingItemMeasurementType
}

// CornerRadius is The nominal radius of a rounded corner measured in the X Y-plane.
type CornerRadius *CornerRadiusType

// CuttingEdgeLengthType is The theoretical length of the cutting edge of a Cutting Item over sharp
//         corners.
type CuttingEdgeLengthType struct {
	*CuttingItemMeasurementType
}

// CuttingEdgeLength is The theoretical length of the cutting edge of a Cutting Item over sharp
//         corners.
type CuttingEdgeLength *CuttingEdgeLengthType

// CuttingHeightType is The distance from the basal plane of the Tool Item to the cutting point.
type CuttingHeightType struct {
	*CuttingItemMeasurementType
}

// CuttingHeight is The distance from the basal plane of the Tool Item to the cutting point.
type CuttingHeight *CuttingHeightType

// CuttingReferencePointType is The theoretical sharp point of the Cutting Tool from which the major
//         functional dimensions are taken.
type CuttingReferencePointType struct {
	*CuttingItemMeasurementType
}

// CuttingReferencePoint is The theoretical sharp point of the Cutting Tool from which the major
//         functional dimensions are taken.
type CuttingReferencePoint *CuttingReferencePointType

// FlangeDiameterType is The dimension between two parallel tangents on the outside edge of a
//         flange.
type FlangeDiameterType struct {
	*CuttingItemMeasurementType
}

// FlangeDiameter is The dimension between two parallel tangents on the outside edge of a
//         flange.
type FlangeDiameter *FlangeDiameterType

// FunctionalWidthType is The distance between the cutting reference point and the rear backing
//         surface of a turning tool or the axis of a boring bar.
type FunctionalWidthType struct {
	*CuttingItemMeasurementType
}

// FunctionalWidth is The distance between the cutting reference point and the rear backing
//         surface of a turning tool or the axis of a boring bar.
...
// IncribedCircleDiameterType is The diameter of a circle to which all edges of a equilateral and round
//         regular insert are tangential.
type IncribedCircleDiameterType struct {
	*CuttingItemMeasurementType
}

// IncribedCircleDiameter is The diameter of a circle to which all edges of a equilateral and round
//         regular insert are tangential.
type IncribedCircleDiameter *IncribedCircleDiameterType

// PointAngleType is The angle between the major cutting edge and the same cutting edge
//         rotated by 180 degrees about the tool axis.
type PointAngleType struct {
	*CuttingItemMeasurementType
}

// PointAngle is The angle between the major cutting edge and the same cutting edge
//         rotated by 180 degrees about the tool axis.
type PointAngle *PointAngleType

// StepDiameterLengthType is The length of a portion of a stepped tool that is related to a
...
// StepDiameterLength is The length of a portion of a stepped tool that is related to a
//         corresponding cutting diameter measured from the cutting reference point
//         of that cutting diameter to the point on the next cutting edge at which
//         the diameter starts to change.
type StepDiameterLength *StepDiameterLengthType

// StepIncludedAngleType is The angle between a major edge on a step of a stepped tool and the same
//         cutting edge rotated 180 degrees about its tool axis.
type StepIncludedAngleType struct {
	*CuttingItemMeasurementType
}

// StepIncludedAngle is The angle between a major edge on a step of a stepped tool and the same
//         cutting edge rotated 180 degrees about its tool axis.
type StepIncludedAngle *StepIncludedAngleType

// ToolCuttingEdgeAngleType is The angle between the tool cutting edge plane and the tool feed plane
//         measured in a plane parallel the xy-plane.
type ToolCuttingEdgeAngleType struct {
	*CuttingItemMeasurementType
}

// ToolCuttingEdgeAngle is The angle between the tool cutting edge plane and the tool feed plane
//         measured in a plane parallel the xy-plane.
type ToolCuttingEdgeAngle *ToolCuttingEdgeAngleType

// ToolLeadAngleType is The angle between the tool cutting edge plane and a plane perpendicular
//         to the tool feed plane measured in a plane parallel the xy-plane.
type ToolLeadAngleType struct {
	*CuttingItemMeasurementType
}

// ToolLeadAngle is The angle between the tool cutting edge plane and a plane perpendicular
//         to the tool feed plane measured in a plane parallel the xy-plane.
type ToolLeadAngle *ToolLeadAngleType

// WiperEdgeLengthType is The measure of the length of a wiper edge of a Cutting Item.
type WiperEdgeLengthType struct {
	*CuttingItemMeasurementType
}

// WiperEdgeLength is The measure of the length of a wiper edge of a Cutting Item.
type WiperEdgeLength *WiperEdgeLengthType

// ToolOrientationType is The angle of the tool with respect to the workpiece for a given process.
...
// CuttingItemsType is A {{block(CuttingItem)}} is the portion of the tool that physically
...
// CuttingItemType is {{block(Measurements)}} {{termplural(organize)}}
...
// AbstractFileType is `FileComments` {{termplural(organize)}} one or more
//                 {{block(FileComment)}} entities for {{block(File)}}s.
type AbstractFileType struct {
	NameAttr                string                   `xml:"name,attr"`
...
// AbstractFile is An {{block(AbstractFile)}} is an abstract {{block(Asset)}} type model
//         that contains the common properties of the {{block(File)}} and
//         {{block(FileArchetype)}} types.
type AbstractFile *AbstractFileType

// FileArchetypeType is {{block(FileArchetype)}} {{block(Asset)}} is an {{block(AbstractFile)}}
//         providing information common to all versions of a file.
type FileArchetypeType struct {
	*AbstractFileType
}

// FileArchetype is {{block(FileArchetype)}} {{block(Asset)}} is an {{block(AbstractFile)}}
//         providing information common to all versions of a file.
type FileArchetype *FileArchetypeType

// FilePropertiesType is A key-value pair providing additional metadata about a
//             {{block(File)}}.
...
// FilePropertyType is A key-value pair providing additional metadata about a {{block(File)}}.
...
// FileType is The time the file was modified.
...
// File is {{block(File)}} {{block(Asset)}} type.
type File *FileType

// FileLocationType is The {{term(URL)}} reference to the file location.
...
// FileDestinationsType is The {{block(Destination)}} is a reference to the target
...
// FileCommentsType is A remark or interpretation for human interpretation associated with
...
// FileCommentType is A remark or interpretation for human interpretation associated with a
