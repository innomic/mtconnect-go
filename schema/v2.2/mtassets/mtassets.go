...
// DefinitionAttrsType is same as {{block(DataItem)}} {{property(units)}}. See {{package(Device
//           Information Model)}}.
...
// MeasurementValueAttrType is A measurement value
type MeasurementValueAttrType string
...
type Measurement *MeasurementTypeType
...
// QifDocumentTypeExtType is Extended tyoe for contained QIF Document type as defined in the QIF
//         Standard.
type QifDocumentTypeExtType string

// QifDocumentTypeEnum is contained QIF Document type as defined in the QIF Standard.
type QifDocumentTypeEnum string

// QifDocumentTypeType is contained QIF Document type as defined in the QIF Standard.
type QifDocumentTypeType struct {
	QifDocumentTypeExtType string
	QifDocumentTypeEnum    string
}

// QIFDocumentWrapperType is {{block(Asset)}} that carries the QIF Document.
type QIFDocumentWrapperType struct {
	QifDocumentTypeAttr *QifDocumentTypeType `xml:"qifDocumentType,attr,omitempty"`
	QIFDocument         *QIFDocumentType     `xml:"QIFDocument"`
	*AssetType
}

// QIFDocumentWrapper is {{block(Asset)}} that carries the QIF Document.
type QIFDocumentWrapper *QIFDocumentWrapperType

// QIFDocumentType is any element
type QIFDocumentType struct {
}

// ContainerTypeType is The type of the material container.
type ContainerTypeType string

// RawMaterialSerialNumberType is The serial number of the material container
type RawMaterialSerialNumberType string

// ProcessKindType is The ISO process type supported by this {{term(raw material)}}
type ProcessKindType string

// RawMaterialMaterialTypeType is The type of material.
type RawMaterialMaterialTypeType string

// ManufacturerType is The name of the raw feed stock manufacturer.
type ManufacturerType string

// ManufacturingDateType is The manufacturing date of the raw feed stock from the feed stock
//         manufacturer.
type ManufacturingDateType string

// ManufacturingCodeType is The lot code of the raw feed stock from the feed stock manufacturer.
type ManufacturingCodeType string

// MaterialCodeType is The ASTM standard code that the material complies with.
type MaterialCodeType string

// LotType is The lot code of the material contained within the material container.
type LotType string

// HasMaterialType is The form of the material in the material container.
type HasMaterialType bool

// FirstUseDateType is The date when the material container was first used.
type FirstUseDateType string

// LastUseDateType is The date when the material container was last used.
type LastUseDateType string

// InitialVolumeType is The amount of material initially placed in the material container when
//         manufactured.
type InitialVolumeType float32

// InitialDimensionType is The dimension of material initially placed in the material container
//         when manufactured.
type InitialDimensionType *FloatListValueType

// InitialQuantityType is The quantity of material initially placed in the material container when
//         manufactured.
type InitialQuantityType int

// CurrentVolumeType is The quantity of material currently contained in the material container
type CurrentVolumeType float32

// CurrentDimensionType is The dimension of material currently contained in the material container
type CurrentDimensionType *FloatListValueType

// CurrentQuantityType is The quantity of material currently contained in the material container
type CurrentQuantityType int

// FormExtType is Extended tyoe for form of the {{term(raw material)}}.
type FormExtType string

// FormEnum is form of the {{term(raw material)}}.
type FormEnum string

// FormType is form of the {{term(raw material)}}.
type FormType struct {
	FormExtType string
	FormEnum    string
}

// RawMaterialMaterialType is {{term(ASTM)}} standard code that the material complies with.
type RawMaterialMaterialType struct {
	IdAttr            string `xml:"id,attr,omitempty"`
	NameAttr          string `xml:"name,attr,omitempty"`
	TypeAttr          string `xml:"type,attr"`
	Lot               string `xml:"Lot"`
	Manufacturer      string `xml:"Manufacturer"`
	ManufacturingDate string `xml:"ManufacturingDate"`
	ManufacturingCode string `xml:"ManufacturingCode"`
	MaterialCode      string `xml:"MaterialCode"`
}

// RawMaterialType is {{block(Resource)}} composed of material that is consumed or
//                 used by the piece of equipment for production of parts,
//                 materials, or other types of goods.
type RawMaterialType struct {
	NameAttr          string                   `xml:"name,attr,omitempty"`
	ContainerTypeAttr string                   `xml:"containerType,attr,omitempty"`
	ProcessKindAttr   string                   `xml:"processKind,attr,omitempty"`
	SerialNumberAttr  string                   `xml:"serialNumber,attr,omitempty"`
	Form              *FormType                `xml:"Form"`
	HasMaterial       bool                     `xml:"HasMaterial"`
	ManufacturingDate string                   `xml:"ManufacturingDate"`
	FirstUseDate      string                   `xml:"FirstUseDate"`
	LastUseDate       string                   `xml:"LastUseDate"`
	InitialVolume     float32                  `xml:"InitialVolume"`
	InitialDimension  *FloatListValueType      `xml:"InitialDimension"`
	InitialQuantity   int                      `xml:"InitialQuantity"`
	CurrentVolume     float32                  `xml:"CurrentVolume"`
	CurrentDimension  *FloatListValueType      `xml:"CurrentDimension"`
	CurrentQuantity   int                      `xml:"CurrentQuantity"`
	Material          *RawMaterialMaterialType `xml:"Material"`
	*AssetType
}

// RawMaterial is {{block(Asset)}} that represents {{term(raw material)}}.
type RawMaterial *RawMaterialType

// ParameterType is single data value that is expected to be reported for a
...
// ParametersType is property defining a configuration of a {{block(Component)}}.
type ParametersType struct {
	Parameter []*ParameterType `xml:"Parameter"`
}

// ParameterSetType is Groups Parameters
type ParameterSetType struct {
	NameAttr   string          `xml:"name,attr"`
	Parameters *ParametersType `xml:"Parameters"`
}

// ParameterSetsType is set of parameters defining the configuration of a
//             {{block(Component)}}.
type ParameterSetsType struct {
	ParameterSet []*ParameterSetType `xml:"ParameterSet"`
}

// ComponentConfigurationParametersType is Groups one or more ParameterSet
type ComponentConfigurationParametersType struct {
	ParameterSets *ParameterSetsType `xml:"ParameterSets"`
	*AssetType
}

// ComponentConfigurationParameters is set of parameters that govern the functionality of the related
//         {{block(Component)}}.
type ComponentConfigurationParameters *ComponentConfigurationParametersType
