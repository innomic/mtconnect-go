...
// EnvironmentalType is {{block(Component)}} that observes the surroundings of another
...
// Environmental is {{block(Component)}} that observes the surroundings of another
//         {{block(Component)}}. > Note: {{block(Environmental)}} **SHOULD** be
//         organized by {{block(Auxillaries)}}, {{block(Systems)}} or
//         {{block(Parts)}} depending on the relationship to the
//         {{block(Component)}}.
...
// FeatureOccurrenceType is {{block(Component)}} that provides information related to an individual
//         {{term(feature)}}.
type FeatureOccurrenceType struct {
	*PartOccurrenceType
}

// FeatureOccurrence is {{block(Component)}} that provides information related to an individual
//         {{term(feature)}}.
type FeatureOccurrence *FeatureOccurrenceType
...
// HrefType is url of an image file
type HrefType string

// MediaTypeType is mime type
type MediaTypeType string

// ImageFilesType is reference to a file containing an image of the
//                 {{block(Component)}}.
type ImageFilesType struct {
	ImageFile []*ImageFileType `xml:"ImageFile"`
	*AbstractConfigurationType
}

// ImageFiles is This section provides semantic information for the {{block(ImageFile)}}
//         entity.
type ImageFiles *ImageFilesType

// ImageFileType is reference to a file containing an image of the {{block(Component)}}.
